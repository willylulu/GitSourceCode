...
mat4
ObjParser::loadObj(char *url){
	std::string err;
	bool ret_obj = tinyobj::LoadObj(shapes, materials, err, url);
	...
	if (ret_obj){
		...
		for (int i = 0; i < materials.size(); i++){
			texture[i] = loadTexture(materials[i].diffuse_texname.c_str());
		}
...
mat4 
ObjParser::getNormalizeMatrix(std::vector<tinyobj::shape_t> shapes) {
	float minVal[3];
